...
public class PagamentoAtrasado implements PagamentoStrategy{
    ...
    public void resultadoPagamento(ContasAPagar conta, Pagamento pg) {
    	...
    	BigDecimal capitalInicialAtualizado = conta.getCapitalInicial().add(conta.getValorDoJuros());
    	conta.setCapitalInicial(capitalInicialAtualizado);
	    ...
	    // 4. Se o pagamento for menor que o valor dos juros (PAGAMENTO INSUFICIENTE PARA COBRIR JUROS)
	    } else { // pg.getValorDoPagamento().compareTo(conta.getValorDoJuros()) < 0
	        throw new RuntimeException("Valor informado no Pagamento é insuficiente para cobrir os juros.");
	    }

        
//        BigDecimal jurosEmReal = conta.getCapitalInicial().multiply(conta.getTaxaDeJuros());
//        conta.setValorDoJuros(jurosEmReal);
//        
//        BigDecimal novoSaldo = conta.getCapitalInicial().add(conta.getValorDoJuros());
//        conta.setSaldoDevedor(novoSaldo);
//
//        if (pg.getValorDoPagamento().compareTo(conta.getValorDoJuros()) > 0) {
//            novoSaldo = conta.getCapitalInicial().subtract(pg.getValorDoPagamento());
////            conta.setSaldoDevedor(novoSaldo);
//        }else if(pg.getValorDoPagamento().equals(conta.getValorDoJuros())){
////            conta.getSaldoDevedor();
//        	conta.getCapitalInicial();
